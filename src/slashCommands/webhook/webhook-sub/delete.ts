...
export default {
    function : async function({ interaction }: { interaction: ChatInputCommandInteraction }) {
      ...
      let infoBtn = new ButtonBuilder()
        .setCustomId("webinfo")
        .setLabel("معلومات الويب")
        .setStyle(ButtonStyle.Primary)

    let deleteBtn = new ButtonBuilder()
        .setCustomId("deleteweb")
        .setLabel("حذف")
        .setStyle(ButtonStyle.Danger)

    let cancelBtn = new ButtonBuilder()
        .setCustomId("cancelweb")
        .setLabel("الغاء")
        .setStyle(ButtonStyle.Secondary)

    let BtnAction = new ActionRowBuilder<ButtonBuilder>().setComponents(infoBtn,deleteBtn, cancelBtn)

    let confirmMsg = await interaction.reply({ephemeral : true , content : `هل انت واثق من انك تريد حذف ${getWebhook.first().name} في ${getWebhook.first().channel}` , components : [BtnAction]})

    let btnCollector = confirmMsg.createMessageComponentCollector({time : 600000,filter : i => ["secret-url","cancelweb" , "deleteweb" , "webinfo" , "urlBtn"].includes(i.customId)})
    ...
    try {
        btnCollector.on("collect" , async buttonInteraction => {
            switch (buttonInteraction.customId) {
                ...
                case "deleteweb":
                    await getWebhook.first().delete();
                    await confirmMsg.edit({content : `تم حذف ${getWebhook.first().name} بنجاح` , components : []})
                    btnCollector.stop()
                ...
                case "webinfo":
                    ...
                    let InfoMsg = await buttonInteraction.reply({ephemeral : true , embeds : [infoEmbed] , components : [urlAction] })
                    break;
